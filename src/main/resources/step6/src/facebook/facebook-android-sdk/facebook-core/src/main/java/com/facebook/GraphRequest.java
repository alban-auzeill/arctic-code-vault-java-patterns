/* (rank 95) copied from https://github.com/facebook/facebook-android-sdk/blob/d1d8d06bfdd70b7a925388ea9385f6319141c9d5/facebook-core/src/main/java/com/facebook/GraphRequest.java
 ...
 *
 * <p>The particular service endpoint that a request targets is determined by a graph path (see the
 * {@link #setGraphPath(String) setGraphPath} method).
 *
 * <p>A Request can be executed either anonymously or representing an authenticated user. In the
 * former case, no AccessToken needs to be specified, while in the latter, an AccessToken must be
 * provided. If requests are executed in a batch, a Facebook application ID must be associated with
 * the batch, either by setting the application ID in the AndroidManifest.xml or via FacebookSdk or
 * by calling the {@link #setDefaultBatchApplicationId(String) setDefaultBatchApplicationId} method.
 *
 * <p>After completion of a request, the AccessToken, if not null and taken from AccessTokenManager,
